# for the cluster data (family data)
import jax
...
def mixed_hmc_on_joint(
    ...
    def get_step_sizes_n_steps(
        ...
        n_repeats = int(npp.ceil(L * n_discrete_to_update / n_discrete_variables))
        #print(n_discrete_to_update)
        #print(n_repeats)
    ...
    def momentum_step_for_ind(
        ...
        if mode == 'RW':
            ...
            potential_array=potential_array.at[jax.ops.index[1]].set(potential(np.where(group==group[jax.ops.index[ind]],0.,q_discrete).at[jax.ops.index[ind]].set(1.),q_continuous))
            ...
            q_discrete_proposal=np.where(proposal_for_ind==1,np.where(group==group[jax.ops.index[ind]],0.,q_discrete).at[jax.ops.index[ind]].set(1),jax.ops.index_update(q_discrete, jax.ops.index[ind], proposal_for_ind))
            delta_E = 0
        else:
            assert False
