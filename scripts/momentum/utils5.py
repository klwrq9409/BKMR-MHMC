...
# cluster 
def generate_bkmr_potential(X,Z, y, crossTT,sigma_a,sigma_b,tau1_a,tau1_b,tau2_a,tau2_b,r_a,r_b, use_jax=True, use_dhmc=False):
...
# def generate_bkmr_potential(X,Z, y, sigma_r,sigma_a,sigma_b,lambda_a,lambda_b,r_a,r_b, use_jax=True, use_dhmc=False):
  ...
  # sigma is a scalar
    ...
    tau1_a = jax.device_put(tau1_a)
    tau1_b = jax.device_put(tau1_b)
    tau2_a = jax.device_put(tau2_a)
    tau2_b = jax.device_put(tau2_b)
    r_a=jax.device_put(r_a)
    r_b=jax.device_put(r_b)
    P=X.shape[1]
    # N=X.shape[0]
    # M=Z.shape[1]
    crossTT=jax.device_put(crossTT)
    ...
    def bkmr_potential(delta,eta):
        ...
        V= sigmasq*jnp.diag(jnp.ones(Z.shape[0])) + tau1*jnp.exp(-Kpart)+tau2*crossTT
        ...
        tau1_prior_potential=tau1_b*tau1-(tau1_a-1)*jnp.log(tau1)
        tau2_prior_potential=tau2_b*tau2-(tau2_a-1)*jnp.log(tau2)
        ...
        potential = gaussian_potential + r_prior_potential + sigmasq_prior_potential + tau2_prior_potential+ tau1_prior_potential+ boundary_potential
