library(bkmr)
files=list.files(pattern = "*knots","/Simulation/N500/m50/bkmr_results/")
...
for (i in index) {
  fit<-readRDS(paste0("/Simulation/N500/m50/bkmr_results/fitkm_bkmr_knots_",i,".rds"))
  timeused[i]<-as.numeric(difftime(fit$time2,fit$time1,units = "mins"))
  hi_est <- ComputePostmeanHnew(fit,sel = 10000:20000)$postmean
  # hi_est
