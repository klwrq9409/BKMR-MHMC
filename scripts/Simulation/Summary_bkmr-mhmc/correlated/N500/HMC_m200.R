...
for (i in 1:100) {
  source_python(paste0("/Simulation/Summary_bkmr-mhmc/correlated/N500/m200/read_data_",i,".py"))
  fit<-list()
  P=ncol(X)
  seqs=2001:5000
  ...
  fit$varsel=TRUE
  class(fit)="bkmrfit"
  fit$groups=c(1,2,1,3:(ncol(Z)-1))
  pipls[i,1:ncol(Z)]=ExtractPIPs(fit)$groupPIP[1:ncol(Z)]
