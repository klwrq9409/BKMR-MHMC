...
txtfiles = []
for file in glob.glob("/Simulation/family_cluster/results.joblib*"):
...
ss=np.stack(d_comb['beta'])
ss.shape
ss2=ss[:,5000:,:]

np.mean(np.mean(ss2,axis=1),axis=0)
np.std(np.mean(ss2,axis=1),axis=0)

np.mean((np.mean(ss2,axis=1)-0.64)**2,axis=0)
...
np.mean(first_true)
np.std(first_true)
timeused=np.array(first_true)/20000*4081.63265
np.mean(timeused)
np.std(timeused)
