...
# A nonlinear and nonadditive function of the first four predictor variables
HFun4 <- function(z, ind = 1:4) 4*plogis(1/4*(z[ind[1]] + z[ind[2]] + 1/2*z[ind[1]]*z[ind[2]]), 0, 0.3)+4*plogis(1/4*(z[ind[3]] + z[ind[4]] + 1/2*z[ind[3]]*z[ind[4]]), 0, 0.3)
# A nonlinear and nonadditive function of the first six predictor variables
HFun5 <- function(z, ind = 1:6) 4*plogis(1/4*(z[ind[1]] + z[ind[2]] + 1/2*z[ind[1]]*z[ind[2]]), 0, 0.3)+4*plogis(1/4*(z[ind[3]] + z[ind[4]] + 1/2*z[ind[3]]*z[ind[4]]), 0, 0.3)+4*plogis(1/4*(z[ind[5]] + z[ind[6]] + 1/2*z[ind[5]]*z[ind[6]]), 0, 0.3)

SimData2 <- function(n = 100, M = 5, sigsq.true = 0.5,
  ...
  if (hfun == 1) {
    ...
    stop("hfun must be an integer from 1 to 4")
  }
