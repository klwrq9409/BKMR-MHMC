...
txtfiles = []
for file in glob.glob("/Simulation/6ind/N500/m50/results/bkmr-mhmc/*"):
    txtfiles.append(file)
# a1=joblib.load("/restricted/projectnb/mricog/Ruiqi/Thesis_hmc/high_corr_Mgtknots/n500_m100/results.joblib_1")
...
for i in range(0,delta.shape[0]): 
    if (np.argwhere((delta_true == delta[i,:,:]).all(1)).shape[0]>0):
        first_true.append(np.argwhere((delta_true == delta[i,:,:]).all(1))[0])


np.where(np.argwhere((delta_true == delta[i,:,:]).all(1)).shape[0]==0)
